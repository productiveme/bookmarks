...
export default function FolderTile(props) {
  ...
  const handleConfirmDelete = (e) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('[FolderTile] Confirm delete for folder:', props.folder.name);
    if (props.onDelete) {
  ...
  const handleCancelDelete = (e) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('[FolderTile] Cancel delete for folder:', props.folder.name);
    setShowDeleteConfirm(false);
  };
  ...
  return (
    <Show
      ...
      fallback={
        <div class="bg-[var(--color-bg-primary)] border-2 border-dashed border-[var(--color-accent)] rounded-lg p-4 hover:shadow-lg transition-all">
          <div class="flex flex-col items-center justify-center gap-3 py-2">
            <span class="text-sm text-[var(--color-text-primary)] text-center">
              Delete folder "{props.folder.name}"?
            </span>
